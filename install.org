...
1. Install the latest release of Python from
   ...
   correctly.  You can then type Ctrl-D to exit the REPL.
...
3. (Optional) Install [[https://visualstudio.microsoft.com/downloads/][VS Code]].
...
+ You may want to brush up on basic command-line usage. Important
  ...
  + ~rm file~ deletes the file (this cannot be undone! it's not the same as
    putting it in the Trash folder)
  + ~rm -r directory_name~ deletes a directory and everything in it
...
+ I generally recommend putting all your code for this course in a
  single directory in, for example, your Documents folder.  You can
  run the command
  #+begin_src none
